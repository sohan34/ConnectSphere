...
public class ChatActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        alumniId = getIntent().getStringExtra("alumniId");
        if (TextUtils.isEmpty(alumniId)) {
            Toast.makeText(this, "Alumni ID not found", Toast.LENGTH_SHORT).show();
            finish();
            return;
        }

        chatRef = firestore.collection("Chats").document(alumniId).collection("Messages");
    ...
    private void sendMessage() {
        ...
        firestore.collection("Users").document(userId).get().addOnCompleteListener(task -> {
            if (task.isSuccessful() && task.getResult() != null) {
                boolean isAlumni = "Alumni".equalsIgnoreCase(task.getResult().getString("userType"));
                messageData.put("isAlumni", isAlumni);
            } else {
                messageData.put("isAlumni", false);
            }
